...
class Module extends Admin
{
    ...
    public function install($name = '', $confirm = 0)
    {
        ...
        if ($ModuleModel->allowField($allowField)
            ->save()) {
            // 复制静态资源目录
    ...
    private function checkDependence($type = '', $data = [])
    {
        ...
        foreach ($data as $key => $value) {
            ...
            // 当前版本
            if ($type == 'module') {
                $curr_version = ModuleModel::where('identifier', $value[1])
                    ->value('version');
            } else {
                $curr_version = PluginModel::where('identifier', $value[1])
                    ->value('version');
            }
    ...
    public function uninstall($name = '', $confirm = 0)
    {
        ...
        // 删除菜单
        if (false === MenuModel::where('module', $name)
                ->delete()) {
            $this->error('菜单删除失败，请重新卸载');
        }

        // 删除授权信息
        if (false === Db::name('admin_access')
                ->where('module', $name)
                ->delete()) {
        ...
        // 删除模块信息
        if (ModuleModel::where('name', $name)
            ->delete()) {
            // 复制静态资源目录
    ...
    public function export($name = '')
    {
        ...
        if (isset($module_info['config'])) {
            $db_config = ModuleModel::where('name', $name)
                ->value('config');
        ...
        // 检查是否有模块行为信息
        $action = Db::name('admin_action')
            ->where('module', $name)
            ->field('module,name,title,remark,rule,log,status')
            ->select();
        if ($action) {
...
INFO;
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $module = ModuleModel::where('id', $ids)
            ->find();
        ...
        if (false !== ModuleModel::where('id', $ids)
                ->setField('status', $status)) {
            // 记录日志
