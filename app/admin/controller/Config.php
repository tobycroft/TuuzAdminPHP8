...
class Config extends Admin
{
    ...
    public function index($group = 'base')
    {
        ...
        // 配置分组信息
        $list_group = config_old('config_group');
        $tab_list = [];
        ...
        return ZBuilder::make('table')
            ...
            ->addColumns([ // 批量添加数据列
                ['name', '名称', 'text.edit'],
                ['title', '标题', 'text.edit'],
                ['type', '类型', 'select', config_old('form_item_type')],
                ['status', '状态', 'switch'],
                ['sort', '排序', 'text.edit'],
                ['right_button', '操作', 'btn']
            ])
            ...
            ->addFilter('type', config_old('form_item_type')) // 添加标题字段筛选
    ...
    public function add($group = '')
    {
        ...
        return ZBuilder::make('form')
            ->setPageTitle('新增')
            ->addRadio('group', '配置分组', '', config_old('config_group'), $group)
            ->addSelect('type', '配置类型', '', config_old('form_item_type'))
    ...
    public function edit($id = 0)
    {
        ...
        return ZBuilder::make('form')
            ->setPageTitle('编辑')
            ->addHidden('id')
            ->addRadio('group', '配置分组', '', config_old('config_group'))
            ->addSelect('type', '配置类型', '', config_old('form_item_type'))
            ->addText('title', '配置标题', '一般由中文组成，仅用于显示')
            ->addText('name', '配置名称', '由英文字母和下划线组成，如 <code>web_site_title</code>，调用方法：<code>config(\'web_site_title\')</code>')
            ->addTextarea('value', '配置值', '该配置的具体内容')
